...
class DummyWorkflow(Workflow):
    async def run(self, context: str) -> WorkflowResult:
        return WorkflowResult(state=ResponseState.NOT_FOUND)

def test_workflow_result_answered_without_decision_raises() -> None:
...
def test_workflow_result_answered_without_authoritative_evidence_raises() -> None:
...
def test_workflow_result_clarification_with_decision_raises() -> None:
...
def test_workflow_result_clarification_without_clarification_raises() -> None:
    with pytest.raises(ValueError, match="CLARIFICATION requires a clarification object"):
        WorkflowResult(
            state=ResponseState.CLARIFICATION
        )

def test_workflow_result_conflict_without_evidence_raises() -> None:
...
def test_workflow_result_not_found_with_decision_raises() -> None:
...
def test_workflow_result_handoff_without_action_raises() -> None:
